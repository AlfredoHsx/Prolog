...
%##################### METODO PRINCIPAL #########################       
    start :-
            ...
            send(Menu,append,new(Iniciar1,popup('Menu: '))),
                    ...
                    send_list(Iniciar1, append,
                        [menu_item('Lista de plantas y su efecto', message(@prolog, pp_listar_planta_efecto))
                        ]),
...
%##################### TARJETA DE INFORMACIÓN DE UNA PLANTA #########################       
    pp_info_planta(Planta):-
        ...
        efecto_de_la_planta(Planta,Efecto),
        ...
        %MostrarEtiquetaDeFormasDeUso
        send(D, display, text('Formas de uso:', left, bold), point(200,200)),nl,
        send(D, display, text(FormasUsoStr, left, normal), point(200,215)),   

        %EtiquetaDeEfectosDeLaPlanta
        send(D, display, text('Efectos:', left, bold), point(200,250)), nl,
        send(D, display, text(Efecto, left, normal), point(200,265)),

        %MostrarEtiquetasDeMedicamentos
        send(D, display, text('Medicamentos que produce:', left, bold), point(200,320)),nl,
        send(D, display, text(MedicamentosStr, left, normal), point(200,335)), 
...
%##################### ENFERMEDADES CURADAS CON LAS PLANTAS #########################       
    pp_enfermedades_curadas_por(Planta):-
        ...
        send(D, display, text('Nombre de la planta: ', center, bold), point(200,30)),
        send(D, display, text(Planta, center, normal), point(350,30)),
        
        %MostrarEtiquetaDeMalestaresParaTratar
        send(D, display, text('Auxiliar en casos de:', center, bold), point(200,60)),
        nl,
...
%#################### LISTA DE ORIGENES #####################
    pp_lista_origenes:-
        new(D, dialog('Lista de origenes de las plntas medicinales')),
        send(D, size, size(500, 450)),
        send(D, colour, colour(black)),  
        % Buscar todas las propiedades y sus efectos
        findall([Planta, Origen], planta_origen(Planta, Origen), Resultados),
        
        % Crear una ventana con scroll
        send(D, append, new(text('Planta -> Lugar de origen:', center, bold))),
        new(W, window('Planta -> Lugar de origen:', size(475, 400))),
        send(W, scrollbars, vertical),  % Agregar barra de scroll vertical
        
        % Mostrar los resultados en la ventana con scroll
        mostrar_resultado_planta_origen(Resultados, W, 10),
        ...
        % Mostrar los resultados en la ventana aplicando recursividad.
        mostrar_resultado_planta_origen([], _, _).
        mostrar_resultado_planta_origen([[Planta, Origen] | Resto], W, Y) :-
        send(W, display, text(Planta, left, bold), point(10, Y)),
        send(W, display, text(Origen, left, normal), point(250, Y)),
        NewY is Y + 20,  % Incrementar la posición Y para la siguiente línea
        mostrar_resultado_planta_origen(Resto, W, NewY).
